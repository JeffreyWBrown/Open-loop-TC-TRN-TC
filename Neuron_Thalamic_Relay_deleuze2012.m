classdef Neuron_Thalamic_Relay_deleuze2012 < handle
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    properties
        Model = 'Neuron Thalamic Relay';
         g_L =  3.2630;  %loss conductance nS
         E_L = -60.0330;          %loss potential mV
        g_Na = 1500;     %fast sodium conductance, ms
        E_Na = 50 ;         %               ms/mm^2
         g_K = 520;      %   potassium rectifier
         E_K = -100;          %    potassium potential 
        g_T = 45.  ;  % t-curret calcium
        E_T = 120 ;           %potential of t current
         g_H = .608;  %matching h-current to leak
         E_H = -33;              %potential of h-current
      
                     %length of neurong
                 %surface area of neuron

        Q = 3.8;    %variable for temperature and Ca++ concentration (Coulter 1989)

        cm = 100.4;        %capacitance of membrane
...
%         
        ...
        Vx = -50.2;          % shift of firing potential
...
%     %integrate potential
          ...
          Vinf = (nn.g_L*nn.E_L + gg_Na*nn.E_Na + ...
            ...
            (nn.g_L + gg_Na + gg_K + gg_T + gg_H + Gs);
